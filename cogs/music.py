...
from discord.commands import slash_command, Option, permissions, errors
...
guild_ids = ['Server ID for commands']
logger = logging.getLogger('testbot')
nonHiddenVC = ('Any voice channel ID that is not hidden to others')
hiddenVC = ('voice channel id that is hidden to others')
testVC = ('any voice channel ids for the testing chanenl')
...
class Music(commands.Cog):
    ...
    async def ensure_voice(self, ctx):
        ...
        should_connect = ctx.command.name in ('play', 'qload', 'tongo')
        ...
        if not player.is_connected:
            ...
            if int(player.channel_id) != ctx.author.voice.channel.id:
                logger.info("User did not run the music command from the same voice channel as the bot")
    ...
    async def song_stuck(self, event):
        if isinstance(event, lavalink.events.TrackStuckEvent):
            ...
            logger.info("The song was stuck so I left the channel.")

    async def new_song(self, event):
        if isinstance(event, lavalink.events.TrackStartEvent):
            logger.info('New Song started')
            ...
            if str(event.player.channel_id) in nonHiddenVC:
                await channel.send(embed=embed)
                return 
            elif str(event.player.channel_id) in hiddenVC:
                return await hiddenChannel.send(embed=embed)
            elif str(event.player.channel_id) in testVC:
    ...
    async def on_ready(self):
        ...
        if not hasattr(self.client, 'lavalink'):  # This ensures the client isn't overwritten during cog reloads.
            ...
            self.client.lavalink.add_node('hostname', 7000, 'password', 'us', 'default-node', 120, "nodename", 5)  # Host, Port, Password, Region, resume key, resume timeout, name, resume attempts

    @slash_command(description="Used to play, queue, or resume a song.", guild_ids=guild_ids)
    @permissions.has_role("DJ")
    ...
    async def stop(self, ctx: ApplicationContext):
        ...
        await ctx.respond('*⃣ | Disconnected.', delete_after=2)
    ...
    async def now(self, ctx: ApplicationContext):
        ...
        await ctx.defer()
        if player.current:
            pos = lavalink.utils.format_time(player.position).lstrip('0:')
            if player.current.stream:
                dur = 'LIVE'
            else:
                dur = lavalink.utils.format_time(player.current.duration).lstrip('0:')
    ...
    async def resume(self, ctx: ApplicationContext):
        ...
        await ctx.defer()
        if not player.paused:
            await ctx.respond('There is nothing paused.')
    ...
    @permissions.has_role("DJ")
    async def remove(self, ctx: ApplicationContext, index: Option(int, "Song to remove", default=None)):
        await ctx.defer()
        player = self.client.lavalink.player_manager.get(ctx.guild.id)
        if index == None:
            return await ctx.respond("You must provide an index to the song you want to remove")

        if not player.queue:
            return await ctx.respond('`Nothing Queued.`')

        if index > len(player.queue) or index < 1:
            return await ctx.respond('Index has to be >=1 and <= the queue size')
    ...
    @permissions.has_role("DJ")
    async def seek(self, ctx: ApplicationContext, time: Option(int, "Time in seconds to seek forward or back(With a -)", default=None)):
        player = self.client.lavalink.player_manager.get(ctx.guild.id)
        randColour = random.randint(0, 0xffffff)
        embed = discord.Embed(color=randColour)
        await ctx.defer()
        if time is None:
            embed.title = "You must provide a time in seconds to seek."
        else:
            if not player.is_playing:
                ...
                if time.startswith('-'):
                    ...
                    if not seconds:
                        embed.title = 'You need to specify the amount you want to skip in seconds'
                    else:
    ...
    async def qload(self, ctx, queue: Option(str, "The queue name")):
        await ctx.defer()
        if queue == None:
            await ctx.respond("You must put in a queue name to load. use the `queues` command to see a list.")
        ...
        if not player.is_playing:
            await player.set_volume(5)
            await player.play()
            msg = await ctx.respond(f"Loaded first song. Loading the rest of: `{queue}`.")
        else:
            await ctx.respond(f'`{queue}` was loaded into the queue.')
        ...
        await ctx.edit_original_message(content=f"`{queue}` has been loaded.")
