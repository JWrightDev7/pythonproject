...
allowed_users = ('UserIDs of users with permission to use the commands')
cmd_allowed = ('channel id of the channels commands are used in')
no_links = ('channel ids that links are not allowed')
roleList = []
roleListDisplay = []

logLocation = "./logs/logs.log"
...
async def load(message, extention):
    owner_id = client.get_user('owner_id')
...
async def unload(message, extention):
    ...
    if message.author.id == 'owner_id':
        ...
        await message.channel.send(f'{message.author.mention} You can not use that command. It is only used by Jim')
...
async def prefix(ctx, pre: str = NULL):
    if len(pre) > 1 and not NULL:
        ...
        logger.info(f'Set the bot prefix to: {prefix}')
...
async def deleted(ctx):
    deletedChannel = client.get_channel('channel_id')
    lastMsg = deletedChannel.last_message_id
    lastMsgContent = await deletedChannel.fetch_message(lastMsg)

    await ctx.send(embed=lastMsgContent.embeds[0])
################################################################## EVENTS ##############################################################
...
async def on_message(message: discord.Message):
    ...
    if re.search(url_regex, message.content) and message.channel.id in no_links:
        if re.search(gifLink, message.content) or re.search(gifLinkTwo, message.content) or re.search(mediaLink, message.content):
            ...
            dest = client.get_channel('channel_id')
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        await ctx.send('I dont have permission to do that, check with Jim to make sure I get the correct permissions.')
        ...
        await ctx.send(f'Something went wrong. Have Jim look into it.')
        logger.warning(f'Someone cause a Data Error: {error}')
